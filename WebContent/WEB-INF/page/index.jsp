					...
					<div class="row">
						...
						<div class="span10">
							...
							<div class="slate clearfix">
								...
								<a class="stat-column" href="#"> <span class="number">${list5}</span>
									<span>用户数量</span>

								</a> <a class="stat-column" href="admin.do"> <span class="number">${list4}</span>
									<span>总发表</span>
								</a> <a class="stat-column" href=""> <span class="number">${list2}</span>
									<span>拾物招领</span>
								</a> <a class="stat-column" href="#"> <span class="number">${list3}</span>
									<span>寻物启事</span>
								</a>
							</div>
						</div>
							...
							<div class="slate">
								...
								<table class="table table-striped">
									...
									<c:forEach var="list" items="${ list }" varStatus="st">
										<tr>
											<td>${fn:substring(list.found_title,0,9)}</td>
											<td>${fn:substring(list.found_category,0,4)}</td>
											<td>${fn:substring(list.found_state,0,4)}</td>
											<td>${fn:substring(list.found_address,0,12)}</td>
											<td>${fn:substring(list.found_name,0,6)}</td>
											<td>${fn:substring(list.found_tel,0,13)}</td>
											<td>${fn:substring(list.found_date,0,20)}</td>
											<td>${fn:substring(list.found_create_date,0,20)}</td>
											
										</tr>
									</c:forEach>
									<c:forEach var="list1" items="${ list1 }" varStatus="st">
										<tr>
											<td>${fn:substring(list1.search_title,0,9)}</td>
											<td>${fn:substring(list1.search_category,0,4)}</td>
											<td>${fn:substring(list1.search_state,0,4)}</td>
											<td>${fn:substring(list1.search_address,0,12)}</td>
											<td>${fn:substring(list1.search_name,0,6)}</td>
											<td>${fn:substring(list1.search_tel,0,13)}</td>
											<td>${fn:substring(list1.search_date,0,20)}</td>
											<td>${fn:substring(list1.send_date,0,20)}</td>
										</tr>
									</c:forEach>
									</tbody>
								</table>
